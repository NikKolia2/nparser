...
class ChromeDriver extends selenium_webdriver_1.Builder {
    constructor() {
        ...
        this.options.setPageLoadStrategy(capabilities_1.PageLoadStrategy.NONE);
        //  this.
    }
    static init() {
    ...
    addExtProxy(proxy) {
        ...
        try {
            fs_1.default.openSync('manifest.json', 'w');
            fs_1.default.writeFileSync('manifest.json', JSON.stringify(manifest_json));
            fs_1.default.openSync('background.js', 'w');
            fs_1.default.writeFileSync('background.js', '' + background_js);
        }
        catch (err) {
            console.error(err);
        }
        ...
        return new Promise((resolve, reject) => {
            zip.archive('proxy_auth_plugin.zip').then(() => {
